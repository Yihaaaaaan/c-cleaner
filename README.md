...
**Windows 最省事的用法**：双击 `C盘清理.bat`，弹出的是一个独立窗口——没有地址栏、
没有标签页，像装了个软件。第一次运行会先扫描（有进度提示），之后秒开；关掉窗口，
后台服务自动退出，不留残留进程。想放到桌面的话，先双击一次 `创建桌面快捷方式.bat`。

桌面窗口用的是系统自带 Edge（或 Chrome）的 `--app` 模式 + 独立 profile，所以仍然
零依赖、不碰你日常浏览器的历史和登录态；机器上没有 Chromium 内核浏览器时自动退回普通浏览器。

命令行等价用法：

```bash
python desktop.py           # 桌面窗口模式（双击入口走的就是它）
python launcher.py          # 控制台菜单：打开上次报告 / 重扫 C 盘 / 扫其他盘，10 秒默认第一项
...
| `main.py` | 一键入口 |
| `desktop.py` | 桌面应用模式：独立窗口（Chromium `--app`）+ tk 启动进度 + 窗口关则收服务 |
| `launcher.py` | 控制台菜单入口、桌面快捷方式生成（ctypes 直调 IShellLinkW） |
